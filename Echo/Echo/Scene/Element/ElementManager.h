...
typedef std::list<CLine*> Lines;
typedef std::list<CLine*>::iterator LinesIt;
...
public:
	void CreateElement(UINT texture, const SMap *map, FLOAT lastTime = 5000.f, 
		BOOL fadeIn = FALSE, DWORD color = 0xFFFFFF, BOOL ui = FALSE, 
		const D3DXVECTOR3& pos = g_vScreenCenter, const D3DXVECTOR3& dir = g_vDefaultDir);
	void CreateLine(const D3DXVECTOR3& pos, const D3DXVECTOR3& dir, 
		const SMap *map, DWORD color = 0xFFFFFF);
	...
	BOOL IsGone() { return m_pAmbience.empty() && m_pAmbience.empty() && m_pElements.empty(); }
	BOOL IsEchoInside(D3DXVECTOR3& pos);
	void Clear();
private:
	Lines m_pAmbience, m_pEcho;
	Elements m_pElements;
