...
HRESULT InitResource(void)
{
	...
	for(int i = 0; i < g_nMapLayout; ++i)
	{
		...
		if(i!=(g_nMapLayout - 1)) // 元素图片
		{
			...
			if(FAILED(hr = g_pDrawBrush[i].OnCreateByPoint(_pd3dDevice,g_pppTexture[i],maxNum)))
			{
				return hr;
			}
		}
...
void CMapEditView::RenderLine()
{
	if(TRUE == m_bIsLine)
	{
		...
		for(int row(0);row<50;++row)
		{
			...
			{
				D3DXVECTOR2(row*16.f, 0.f),
				D3DXVECTOR2(row*16.f, 600.f)
			};
		...
		for(int col(0);col<38;++col)
		{
			D3DXVECTOR2 vecLine[2] = 
			{
				D3DXVECTOR2(0.f, col*16.f),
				D3DXVECTOR2(800.f, col*16.f)
			};
			_pLine->Draw(vecLine, 2, 0x4000FF00);
		}
		_pLine->End();
...
HRESULT CMapEditView::Render()
{
	...
	if(_pd3dDevice)
	{
		...
		{
			// 绘制背景层
			g_pDrawBrush[CURSOR_BACK].Render(NULL,m_nMapCoord,FALSE);
			g_pDrawBrush[CURSOR_SHADOW].Render(NULL,m_nMapCoord,FALSE);
			...
			if(m_emMapLayout != MAP_ALL)
			{
				...
				if(g_pDrawBrush && (m_emCursorLayout != CURSOR_NONE))
				{
					...
					if(m_emCursorLayout == CURSOR_COLL || m_emCursorLayout == CURSOR_TRIG)
						g_pDrawBrush[m_emCursorLayout].Render(NULL,m_nDefaultCoord,FALSE);
					else
						g_pDrawBrush[m_emCursorLayout].Render(_pLine,m_nDefaultCoord,FALSE);
				}
			}
	...
	if(m_bIsNewMap) {
		...
		if(m_emMapLayout == MAP_COLL && g_pDrawBrush[CURSOR_COLL].GetActive()) {
			g_pDrawBrush[CURSOR_COLL].SetTexturePosition(
				(float)point.x + g_pDrawBrush[CURSOR_COLL].GetTextureWidth()/4,
				(float)point.y + g_pDrawBrush[CURSOR_COLL].GetTextureHeight()/4
			);
			if(m_bIsLBotton) {
				// 判断鼠标在当前碰撞数组的几行几列
				m_pDrawMapColl->Update(point.x+m_nMapCoord.x,point.y+m_nMapCoord.y);
			}
		} else if(m_emMapLayout == MAP_TRIG) {
			for(size_t i(0);i<m_stdMapTrigElement.size();++i) {
				if(m_stdMapTrigElement[i]->GetSelectState()) {
					m_stdMapTrigElement[i]->SetTexturePosition(
						(float)point.x+m_nMapCoord.x, (float)point.y+m_nMapCoord.y);
					break;
				}
			}
		} else if(m_emCursorLayout != CURSOR_NONE) {
			g_pDrawBrush[m_emCursorLayout].SetTexturePosition((float)point.x,(float)point.y);
		}
	}
...
void CMapEditView::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	else if(m_emMapLayout == MAP_COLL) // 如果是碰撞层状态
	{
		...
		if(g_pDrawBrush[CURSOR_COLL].GetActive()) 
		{
			...
			{
				// 判断鼠标在当前碰撞数组的几行几列
				m_pDrawMapColl->Update(point.x,point.y);
			}
		}
	}
...
void CMapEditView::LoadFileByBin(std::wstring PathName)
{
	...
	//设置阴影图片
	g_pDrawBrush[CURSOR_SHADOW].SetTextureNum(backNum);
	g_pDrawBrush[CURSOR_SHADOW].SetTextureColor(D3DCOLOR_ARGB(BACK_DEFAULT_ALPHAVALUE,255,255,255));
	g_pDrawBrush[CURSOR_SHADOW].SetActive();
