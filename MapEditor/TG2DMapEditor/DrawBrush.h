...
public:   // 公有调用函数
	HRESULT OnCreateByPoint(LPDIRECT3DDEVICE9 pd3dDevice, LPDIRECT3DTEXTURE9* ppTexture, INT num, BOOL bIsActive = FALSE);
	HRESULT Render(LPD3DXLINE pLine,POINT& offset,BOOL zDepth = TRUE);
	void  Destroy();
public:   // 内联函数
	inline void  SetTextureNum(INT num){ m_nCurNum = num; }
	inline void  SetTexturePosition(float x, float y) { 
		m_matWorld._41 = x-GetTextureWidth()/2;
		m_matWorld._42 = y-GetTextureHeight()/2; }
	...
	inline float GetPositionX(){ return m_matWorld._41; }
	inline float GetPositionY(){ return m_matWorld._42; }
